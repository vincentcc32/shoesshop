    ...
    <div class="container pb-5 min-h-70vh">
        <div style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="my-5 pt-5">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active fs-3" aria-current="page">Category Fahion</li>
            </ol>
        </div>
        <div class="category-shoes-filter">
            <div class="product-filter d-flex justify-content-end align-items-center border-bottom pb-5">
                ...
                <button class="btn text-bg-dark ms-3 py-2 px-4 fs-5 btn-cate-fashion">Filter</button>
            </div>
        </div>
                ...
                <ul class="category-shoes-list p-0">
                    <?php foreach ($dssm as $dm) : ?>
                        <li class="d-block py-2">
                            <a href="?ctrl=product&view=categoryfashion&make=<?= $dm['MakeInID'] ?>" class="d-block py-2 fs-4 text-decoration-none text-dark"><?= $dm['Name'] ?></a>
                            <input type="text" hidden name="fashionmake" value="<?= $_GET['make'] ?>">
                        </li>
                    <?php endforeach; ?>
                    <li class="d-block py-2">
                        <a href="?ctrl=product&view=categoryfashion&make=0" class="d-block py-2 fs-4 text-decoration-none text-dark text-uppercase">other fashion</a>
                    </li>
                </ul>
            </div>
                            ...
                            <div class="content__product-box position-relative">
                                <a href="?ctrl=product&view=detail&sp=<?= $sp['Slug'] ?>" class="content__product-link text-decoration-none">
                                    ...
                                    <div class="content__product-img">
                                        <img src="./public/images/uploads/<?= $sp['Image'] ?>" alt="" class="w-100 object-fit-cover">
                                    </div>
            ...
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    <?php if (isset($_GET['page'])): ?>
                        <?php if (isset($_GET['sort'])): ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=categoryfashion&make=<?= $_GET['make'] ?><?= $_GET['page'] === '1' ? "" : "&page=" . $index - 1 ?>&sort=<?= $_GET['sort'] ?>" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        <?php else : ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=categoryfashion&make=<?= $_GET['make'] ?><?= $_GET['page'] === '1' ? "" : "&page=" . $index - 1 ?>" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>
                ...
                <?php if (isset($_GET['sort'])): ?>
                    <?php for ($j = 1; $j < $i; $j++): ?>
                        <li class="page-item"><a class="page-link fs-3 px-3 text-dark <?= isset($_GET['page']) && (int)$_GET['page'] === $j ? 'active' : '' ?>" href="index.php?ctrl=product&view=categoryfashion&make=<?= $_GET['make'] ?>&page=<?= $j ?>&sort=<?= $_GET['sort'] ?>"><?= $j ?></a></li>
                    <?php endfor; ?>
                <?php else : ?>
                    <?php for ($j = 1; $j < $i; $j++): ?>
                        <li class="page-item"><a class="page-link fs-3 px-3 text-dark <?= isset($_GET['page']) && (int)$_GET['page'] === $j ? 'active' : '' ?>" href="index.php?ctrl=product&view=categoryfashion&make=<?= $_GET['make'] ?>&page=<?= $j ?>"><?= $j ?></a></li>
                    <?php endfor; ?>
                <?php endif; ?>
                <li class="page-item">
                    <?php if ($i > 1 && $index + 1 < $i): ?>
                        <?php if (isset($_GET['sort'])): ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=categoryfashion&make=<?= $_GET['make'] ?>&page=<?= $index + 1 ?>&sort=<?= $_GET['sort'] ?>" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        <?php else : ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=categoryfashion&make=<?= $_GET['make'] ?>&page=<?= $index + 1 ?>" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>
