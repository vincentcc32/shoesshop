    ...
    <div class="container min-h-70vh">
        <div style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="my-5 pt-5">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active fs-3" aria-current="page">Fashion</li>
            </ol>
        </div>
        <div class="product-filter d-flex justify-content-end align-items-center border-bottom pb-5">
            ...
            <button class="btn text-bg-dark ms-3 py-2 px-4 fs-5 btn-fashion-filter">Filter</button>
        </div>
        <div class="row pt-5 mt-5 g-5">
            <?php if ($dsspFashionAll): ?>
                <?php foreach ($dsspFashionAll as $sp) : ?>
            ...
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    <?php if (isset($_GET['page'])): ?>
                        <?php if (isset($_GET['sort'])): ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=fashion<?= $_GET['page'] === '1' ? "" : "&page=" . $index - 1 ?>&sort=<?= $_GET['sort'] ?>" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        <?php else : ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=fashion<?= $_GET['page'] === '1' ? "" : "&page=" . $index - 1 ?>" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>
                ...
                <?php if (isset($_GET['sort'])): ?>
                    <?php for ($j = 1; $j < $i; $j++): ?>
                        <li class="page-item"><a class="page-link fs-3 px-3 text-dark <?= isset($_GET['page']) && (int)$_GET['page'] === $j ? 'active' : '' ?>" href="index.php?ctrl=product&view=fashion&page=<?= $j ?>&sort=<?= $_GET['sort'] ?>"><?= $j ?></a></li>
                    <?php endfor; ?>
                <?php else : ?>
                    <?php for ($j = 1; $j < $i; $j++): ?>
                        <li class="page-item"><a class="page-link fs-3 px-3 text-dark <?= isset($_GET['page']) && (int)$_GET['page'] === $j ? 'active' : '' ?>" href="index.php?ctrl=product&view=fashion&page=<?= $j ?>"><?= $j ?></a></li>
                    <?php endfor; ?>
                <?php endif; ?>
                <li class="page-item">
                    <?php if ($i > 1 && $index + 1 < $i): ?>
                        <?php if (isset($_GET['sort'])): ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=fashion&page=<?= $index + 1 ?>&sort=<?= $_GET['sort'] ?>" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        <?php else : ?>
                            <a class="page-link fs-3 px-3 text-dark" href="index.php?ctrl=product&view=fashion&page=<?= $index + 1 ?>" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>
